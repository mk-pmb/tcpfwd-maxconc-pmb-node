...
function maybeInstallPeekData(maxLen, lucky, fwd, logMisc, logPfx) {
  if (!maxLen) { return; }
  function peekData(side, data) {
    var ellip = '';
    ...
    data = data.replace(/[\x00-\x1F%\x7F-\uFFFF]+/g, encodeURIComponent);
    logMisc(logPfx + 'Greeting from ' + side + ': ‹' + data + '›' + ellip);
  }
  lucky.once('data', peekData.bind(null, 'seat'));
  fwd.once('data', peekData.bind(null, 'target'));
}
...
function install(srv, cfg) {
  ...
  SQ.checkSeatAvail = function () {
    ...
    maybeInstallPeekData(cfg.peekData, lucky, fwd, logMisc, logPfx);
